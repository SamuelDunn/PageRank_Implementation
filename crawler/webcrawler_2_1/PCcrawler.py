...
'''
def print_header_record(filestream, num_page, page_size, timestamp, \
               url,canonical_url):
    
    print >> filestream,  "* ", str(num_page)+ ':' + str(page_size) + ':' + \
                          timestamp, 
    try: 
       print >> filestream,  url
    except UnicodeEncodeError:
       print >> filestream,  "$" +  \
               url.encode('ascii', 'xmlcharrefreplace')

    if (url != canonical_url) :
      print >> filestream,  "# ", canonical_url
...
def safe_print_url(filestream, url):
    try: 
        ...
        print >> filestream, "$" + url.encode('ascii', 'xmlcharrefreplace')


def print_links(filestream, page_links):
    for link in page_links:
       try: 
         print >> filestream,  link
       except UnicodeEncodeError:
         print >> filestream, "$" + link.encode('ascii', 'xmlcharrefreplace')
    filestream.flush()  
...
#
  print >> filestream,  " "
  print >> filestream,  "[-- Frontier --]" 
  for edge in links_to_visit:
      print >> filestream,  edge
  print >> filestream,  "[-- Frontier end --]" 
...
def modular_process_webpage( num_page, \
    ...
    if (page_contents in url_errors.URL_errors):
        print_error_record(filestream, num_page, seq_timestamp, url, \
                  canonical_url, page_contents)
...
#
##################################################
def process_webpage( num_page, timestamp, url, canonical_url, \
    ...
    #
    if (page_contents in url_errors.URL_errors):
        print_error_record(filestream, num_page, timestamp, url, \
                  canonical_url, page_contents)
        return []
    ...
    print_header_record(filestream, num_page, len(page_contents), timestamp, \
                   url, canonical_url )
    ...
    print_links(filestream, follow_links)
    print ''
...
def consumer( filestream, url_matching_pattern, max_num_page_visited,   \
  ...
  while (len(links_to_visit) > 0) and \
    ...
    timestamp,canonical_url,page_contents = producer(url, \
    ...
    links_to_follow = process_webpage(num_page, timestamp, url, \
         canonical_url, page_contents, links_already_dispatched)
